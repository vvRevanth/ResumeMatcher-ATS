...
st.set_page_config(
    ...
    page_icon="‍",  # Use a placeholder for now
    layout="centered",
)

# Configure Gemini AI model with the API key from environment variable
api_key = os.getenv("AIzaSyD2oLQHkz9sYQvKZN6VaZ7ZI2t2N79wefQ")
...
and big data engineering. Your task is to evaluate the resume based on the given job description and skills.
...
Assign the percentage Matching based on JD and the missing skills/keywords with high accuracy.
resume:{text}
description:{jd}
skills:{skills}

I want the response in one single string having the structure
{{"JD Match":"%","MissingSkills/Keywords":[],"Profile Summary":""}}
"""
...
skills = st.multiselect("Select Skills", ["Python", "Java", "JavaScript", "SQL", "C++", "Machine Learning", "Data Analysis"])
...
if submit:
    if uploaded_file is not None:
        ...
        response = get_gemini_response(input_prompt.format(text=text, jd=jd, skills=", ".join(skills)))
